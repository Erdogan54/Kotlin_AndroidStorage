...
class MainActivity : AppCompatActivity() {
    ...
    private var writePermissinGranted=false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        internalStoragePhotoAdapter= InternalStoragePhotoAdapter {
            lifecycleScope.launch {
                ...
                if (isDeletionSuccessfully){
                    loadPhotosFromInternalStorageIntoReycylerView()
        ...
        permissionsLauncher=registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()){
            ...
            writePermissinGranted=it[android.Manifest.permission.WRITE_EXTERNAL_STORAGE] ?: writePermissinGranted

            if (readPermissionGranted){
                loadPhotosFromExternalStorageIntoReycylerView()
            }else{
        ...
        intentSenderLauncher=registerForActivityResult(ActivityResultContracts.StartIntentSenderForResult()){
            if (it.resultCode== RESULT_OK){
                ...
                Toast.makeText(this,"Photo couldn't be deleted",Toast.LENGTH_LONG).show()
        ...
        val takePhoto=registerForActivityResult(ActivityResultContracts.TakePicturePreview()){
            lifecycleScope.launch {
                ...
                val isSavedSuccessfully=when{
                    ...
                    writePermissinGranted->savePhotoToExternalStorage(UUID.randomUUID().toString(),it)
                    else->false
                }
    ...
    private fun updateOrRequestPermission(){
        ...
        writePermissinGranted=hasWritePermission || minSdk29

        val permissionsToRequest= mutableListOf<String>()
        if (!readPermissionGranted){
            permissionsToRequest.add(android.Manifest.permission.READ_EXTERNAL_STORAGE)
        }
        if (!writePermissinGranted){
    ...
    private fun setupInternalStorageRecyclerView()=binding.rvPublicPhotos.apply {
    ...
    private fun loadPhotosFromInternalStorageIntoReycylerView(){
        lifecycleScope.launch {
    ...
    private fun loadPhotosFromExternalStorageIntoReycylerView(){
        lifecycleScope.launch {
        ...
        return withContext(Dispatchers.IO){
            try {
                openFileOutput("$filename.jpg", MODE_PRIVATE).use {
                    if(!bmp.compress(Bitmap.CompressFormat.JPEG,96,it)){
                        throw IOException ("Couldn't save bitmap.")
                    }
                }
                true
